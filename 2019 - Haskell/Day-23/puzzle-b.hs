...
bootNetwork :: M.Map Int Int -> Int -> [Int]
bootNetwork program machinesCount = relayPackets 0 network 0 []
    where network = M.fromAscList $ zip [0..] $ map (\i -> (program, 0, 0, i:cycle [-1])) [0..(machinesCount-1)]

          relayPackets machine network idleCount natPacket
                | idleCount == machinesCount && not (null natPacket) = traceShow natPacket $ let (mem, base, ip, inputs) = network ! machine
                                                                                                 state = Just (natPacket, mem, base, ip, inputs)
                                                                                             in sendPacket machine network 0 [] state
                | otherwise = let (mem, base, ip, inputs) = network ! machine
                                  state = computeNOutputs 3 (2^10) mem base ip inputs []
                                  idleCount' = if machine == 0 then 0 else idleCount
                              in sendPacket machine network idleCount' natPacket state

          sendPacket machine network idleCount natPacket Nothing = []
          sendPacket machine network idleCount natPacket state = case state of
                                             Just ([addr, x, y], mem', base', ip', inputs') -> case addr of
                                                255 -> let network' = M.insert machine (mem', base', ip', inputs') network
                                                       in relayPackets ((machine + 1) `mod` machinesCount) network' idleCount [0, x, y]
                                                _ -> let network' = M.insert machine (mem', base', ip', inputs') network
                                                     ...
                                                     in relayPackets ((machine + 1) `mod` machinesCount) network'' idleCount natPacket
                                             Just ([], mem', base', ip', inputs') -> let network' = M.insert machine (mem', base', ip', inputs') network
                                                                                         isIdle = if head inputs' == -1 then 1 else 0
                                                                                     in relayPackets ((machine + 1) `mod` machinesCount) network' (idleCount + isIdle) natPacket
