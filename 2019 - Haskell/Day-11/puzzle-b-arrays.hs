...
parse input extraMem = listArray (0, (length program) - 1 + extraMem) $ program ++ replicate extraMem 0
    where program = map read (words [if c == ',' then ' ' else c | c <- input])

directAddr :: Array Int Int -> Int -> Int
directAddr memory addr = memory ! addr

indirectAddr :: Array Int Int -> Int -> Int -> Int
indirectAddr memory addr base = memory ! ((directAddr memory addr) + base)

getParam :: Array Int Int -> Int -> Int -> Int -> Int
...
decodeInstruction :: Array Int Int -> Int -> (Int, Int, Int, Int)
...
computeBinaryOp :: Array Int Int -> Int -> Int -> (Int -> Int -> Int) -> (Int, Int, Int, Int) -> Array Int Int
computeBinaryOp memory base ip op (_, mod1, mod2, mod3) = memory // [(saveTo, leftHand  `op` rightHand)]
...
input :: Array Int Int -> Int -> Int-> Int -> Int -> Array Int Int
input memory base ip mod value = memory // [(saveTo, value)]
    where saveTo = (getParam memory (ip + 1) 1 0) + (if mod == 2 then base else 0)

output :: Array Int Int -> Int -> Int -> (Int, Int, Int, Int) -> Int
output memory base ip (_, mod1, _, _) = getParam memory (ip + 1) mod1 base

jump :: Bool -> Array Int Int -> Int -> Int -> (Int, Int, Int, Int) -> Int
...
computeNextOutput :: Array Int Int -> Int -> Int -> [Int] -> Maybe (Int, Array Int Int, Int, Int, [Int])
...
computeProgram :: Array Int Int -> Int -> Int -> [Int] -> [Int] -> [Int]
...
runRobot :: (Int, Int) -> [(Int, Int)] -> Array Int Int -> Int -> Int -> [Int] -> [((Int, Int), Int)] -> [((Int, Int), Int)]
...
main = do
    input <- getContents
    mapM print $ readOutput $ runRobot (0, 0) (getDirs) (parse input 128000) 0 0 [1] []
