...
--longestPath :: [Int] -> Maybe (Distance, Pos)
longestPath program = bfsSearch 0 ((S.singleton (0, (0, 0), (1, program, 0, 0))), M.singleton (0, 0) 1)
    where notVisited visitedMap (pos, _) = not $ pos `M.member` visitedMap
          visitNeigbors pos visitedMap (mem, base, ip) =  map (fmap (fromJust . computeNextOutput mem base ip)) $ filter (notVisited visitedMap . mapMove pos) moves
          ...
          bfsSearch maxDist (S.Empty, _) = maxDist 
          bfsSearch maxDist ((curr@(dist, pos, (out, mem, base, ip)) :<| q), visitedMap)
            | out == 2 = bfsSearch 0 ((S.singleton (0, pos, (1, mem, base, ip))), M.singleton pos 1)
            | otherwise = bfsSearch (max dist maxDist) $! foldl (processVisited curr) (q, visitedMap) $ visitNeigbors pos visitedMap (mem, base, ip)


main :: IO ()
main = do
    program <- fmap parse getContents
    print $ longestPath program
