...
countOutput number  output  = length . filter (== number) $ snd . unzip $ filter ((==0) . (`mod` 3) . fst) $ zip [1..] output
...
main = do
    ...
    print $ computeProgram (parse input ++ replicate 128000 0) 0 0 (repeat 0) []
