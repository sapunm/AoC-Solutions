...
playArcade input = do
    ...
    where gameStep :: IORef ([Int], Int, Int) -> IO ()
          gameStep state = do
              (memory, base, ip) <- readIORef state
              result <- computeNOutputsInteractive 3 memory base ip mapToInput []
              case result of
                Nothing -> return ()
                Just (outputs, memory', base', ip') -> do
                    ...
                    then return ()
                        ...
                        mapToOutput outputs
                        -- print outputs
                        -- setCursorPosition (outputs !! 1) (outputs !! 0)
                        -- print (outputs !! 2)
